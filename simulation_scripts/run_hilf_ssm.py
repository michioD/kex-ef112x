...
def run_hierarchical_inference_simulation(
    image_paths,
    output_csv="hilf_results_ssm.csv",
    ...
    for t, img_path in enumerate(image_paths):
        ...
        decision_success += 1 if (Y_t > 0 and not accept_sml_cluster) or (Y_t == 0 and accept_sml_cluster) else 0

        correct_accept += 1 if accept_sml_cluster and Y_t == 0 else 0
        incorrect_accept += 1 if accept_sml_cluster and Y_t > 0 else 0
        correct_offload += 1 if not accept_sml_cluster and Y_t > 0 else 0
        incorrect_offload += 1 if not accept_sml_cluster and Y_t == 0 else 0
        ...
        #     writer = csv.writer(f)
        #     writer.writerow([t, s_t, q_t_cluster, action, Y_t, success, fp, fn, misclassified])
