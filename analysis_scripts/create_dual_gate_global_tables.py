#!/usr/bin/env python3
"""
analysis_scripts/create_dual_gate_global_tables.py

Recreates Table V (Table 5) and Table VI (Table 6) from the thesis report:
- Table V: Decision Performance for Dual-Gate (Global)
- Table VI: Correct Offloads Breakdown Dual-Gate (Global)

Data source:
  results/hilf_results_dual_gate_same_yt.csv
"""
...
def ensure_results_exist(csv_path: Path):
    if not csv_path.exists():
        ...
        hilf_dual_gate_global.run_hierarchical_inference_simulation(image_paths, output_csv=str(csv_path))


def generate_tables(csv_path: Path = CSV_DUAL_GLOBAL) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    table5 = pd.DataFrame(matrix_rows)
    table6 = pd.DataFrame(breakdown_rows)
    return table5, table6


def format_table5_latex(df: pd.DataFrame) -> str:
    lines = [
        "\\begin{table}[ht]",
        "\\centering",
        "\\scriptsize",
        "\\caption{Decision Performance for Dual-Gate (Global)}",
    ...
    for _, row in df.iterrows():
        ...
        lines.append(f"{label:<13} & {row['Correct Accept']:<3} & {row['Incorrect Accept']:<2} & {row['Correct Offload']:<4} & {row['Incorrect Offload']} \\\\")
    lines.extend([
        "\\bottomrule",
        "\\end{tabular}",
        "\\label{tab:dual_global_matrix}",
        "\\end{table}",
    ])
    return "\n".join(lines)


def format_table6_latex(df: pd.DataFrame) -> str:
    lines = [
        "\\begin{table}[!ht]",
        "\\centering",
        "\\footnotesize",
        "\\caption{Correct Offloads Breakdown Dual-Gate (Global)}",
        "\\begin{tabular}{@{}lcccc@{}}",
        "\\toprule",
    ...
    lines.extend([
        "\\bottomrule",
        "\\end{tabular}",
        "\\label{tab:global_rejection_failure_sources}",
        "\\end{table}",
    ])
    return "\n".join(lines)
...
def main():
    parser = argparse.ArgumentParser(description="Recreate Table V (5) and Table VI (6) for Dual-Gate (Global).")
    parser.add_argument("--csv", type=Path, default=CSV_DUAL_GLOBAL, help="Path to hilf_results_dual_gate_same_yt.csv")
    parser.add_argument("--latex", action="store_true", help="Output tables in LaTeX format.")
    args = parser.parse_args()

    table5, table6 = generate_tables(args.csv)

    if args.latex:
        print(format_table5_latex(table5))
        print("\n")
        print(format_table6_latex(table6))
    else:
        sep = "=" * 78
        print(f"\n{sep}")
        print("Table V (Table 5): Decision Performance for Dual-Gate (Global)".center(78))
        print(f"{sep}\n")
        print(table5.to_string(index=False))

        print(f"\n\n{sep}")
        print("Table VI (Table 6): Correct Offloads Breakdown Dual-Gate (Global)".center(78))
        print(f"{sep}\n")
